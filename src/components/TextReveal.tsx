...
export function TextReveal({ visible }: TextRevealProps) {
  ...
  useFrame(() => {
    ...
    if (visible && scale < 1) {
      // Pop-in scale animation easing
      setScale((prev) => Math.min(1, prev + (1 - prev) * 0.15));
    }
    ...
    if (mainTextRef.current && subTextRef.current) {
      ...
      mainTextRef.current.scale.setScalar(scale);
      subTextRef.current.scale.setScalar(scale);
    }
  });
