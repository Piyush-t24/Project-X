...
const STONE_COLORS = ['#ff4444', '#4444ff', '#44ff44', '#ffff44', '#ff44ff', '#ff8844'];
...
export function CosmicScene() {
  const [animationPhase, setAnimationPhase] = useState<'approaching' | 'colliding' | 'exploding' | 'revealed'>('approaching');
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      if (animationPhase === 'approaching' && elapsed < 3) {
        ...
        startTimeRef.current = Date.now();
      } else if (animationPhase === 'colliding' && elapsed >= 0.5 && !hasExploded) {
  ...
  return (
    <div className="fixed inset-0 w-full h-full bg-black">
      <Canvas className="w-full h-full">
        ...
        {!explosionComplete && STONE_COLORS.map((color, index) => (
          <GlowingStone
            key={index}
            color={color}
        ...
        {!explosionComplete && <ExplosionEffect 
